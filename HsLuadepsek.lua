...
═══════════════════════════════════════════════════════════════════════
    ...
    FIXES    : Delta mobile writefile support, UI touch improvements
...
-- ========== FIX: Delta mobile writefile helper ==========
local function getExecutorWorkspacePath()
    -- Detect executor and return proper workspace path
    local exec = "unknown"
    pcall(function() exec = identifyexecutor() or "unknown" end)
    exec = tostring(exec):lower()
    
    if exec:find("delta") then
        -- Delta on Android: writefile writes to /sdcard/Delta/workspace/
        -- but relative paths work if we prepend "workspace/"
        return "workspace/"
    elseif exec:find("arceus") then
        return "Arceus/"
    elseif exec:find("hydrogen") then
        return "Hydrogen/"
    elseif exec:find("krnl") or exec:find("fluxus") then
        return ""
    else
        return ""
    end
...
local function safeWriteFile(filename, data)
    local success = false
    local errMsg = ""
    local finalPath = filename
    
    -- Ensure folder exists
    local folder = getExecutorWorkspacePath()
    if folder ~= "" then
    ...
    -- Try writefile
    if writefile then
        success, errMsg = pcall(writefile, finalPath, data)
        if success then
            return true, finalPath
        end
    end
    
    -- Fallback: try syn.writefile
    if syn and syn.writefile then
        success, errMsg = pcall(syn.writefile, finalPath, data)
        if success then
            return true, finalPath
        end
    end
    
    -- Last resort: set clipboard and notify
    pcall(function()
        if setclipboard then setclipboard(data) end
    end)
    return false, errMsg or "writefile unavailable, data copied to clipboard"
end
...
Stealth.Cap = {
    ...
    writefile       = type(writefile) == "function" or (syn and type(syn.writefile) == "function"),
    readfile        = type(readfile) == "function" or (syn and type(syn.readfile) == "function"),
...
Stealth.writefile  = writefile or (syn and syn.writefile) or function() end
Stealth.readfile   = readfile or (syn and syn.readfile) or function() return nil end
...
-- Use safeWriteFile from outer scope but defined after; we'll wrap
local function _safeWriteFile(filename, data)
    if safeWriteFile then return safeWriteFile(filename, data) end
    -- fallback
    if writefile then
        return pcall(writefile, filename, data)
    end
...
-- ========== FIXED WRITEFILE EXPORT (global for telemetry) ==========
local function safeWriteFile(filename, data)
    local folder = ""
    -- detect executor for Delta mobile
    local exec = "unknown"
    pcall(function() exec = identifyexecutor() or "unknown" end)
    exec = tostring(exec):lower()
    ...
    if exec:find("delta") then
        folder = "workspace/"
        pcall(function()
            if makefolder then makefolder("workspace") end
        end)
    elseif exec:find("arceus") then
        folder = "Arceus/"
        pcall(function() if makefolder then makefolder("Arceus") end end)
    elseif exec:find("hydrogen") then
        folder = "Hydrogen/"
        pcall(function() if makefolder then makefolder("Hydrogen") end end)
    end
    
    local path = folder .. filename
    local success = false
    if writefile then
        success = pcall(writefile, path, data)
    elseif syn and syn.writefile then
        success = pcall(syn.writefile, path, data)
    end
    return success, path
end
...
local Window = HSHub:CreateWindow({
    ...
    Tag='HS-COS-V4-FIX', ToggleKey='RightShift',
})
...
do
    ...
    CS:AddSlider({ Name='Turn Radius', Key='TR', Min=0, Max=200, Default=0, Step=1, Callback=function(v) S.TurnRadius=v end })
    CS:AddToggle({ Name='Enable Custom Turn Radius', Key='ETR', Default=false, Callback=function(v) S.EnableTurnRadius=v end })
    CS:AddSlider({ Name='Walk Speed', Key='WS', Min=0, Max=200, Default=30, Step=1, Callback=function(v) S.WalkSpeed=v end })
    CS:AddToggle({ Name='Enable Custom Walk Speed', Key='EWS', Default=false, Callback=function(v) S.EnableWalkSpeed=v end })
    CS:AddSlider({ Name='Sprint Speed', Key='SS', Min=0, Max=300, Default=115, Step=1, Callback=function(v) S.SprintSpeed=v end })
    CS:AddToggle({ Name='Enable Custom Sprint Speed', Key='ESS', Default=false, Callback=function(v) S.EnableSprintSpeed=v end })
    CS:AddSlider({ Name='Fly Speed', Key='FS', Min=0, Max=200, Default=40, Step=1, Callback=function(v) S.FlySpeed=v end })
...
do
    ...
    R:AddSlider({ Name='Death Points Target', Key='DPT', Min=0, Max=5000, Default=1200, Step=1,
...
do
    ...
    local saveName = ''
    -- No textbox in base UI, use label + button combo
    Cu:AddLabel('Enter name in console:')  -- placeholder
    Cu:AddButton({ Name='Save Current Location', Callback=function()
        local root = getRoot(); if not root then return end
        local name = tostring(math.random(1000,9999))
        locs[name]=root.Position
        HSHub:Notify('Saved: '..name,'ok',2)
        -- refresh dropdown options
        local opts = {''}
        for k in pairs(locs) do table.insert(opts, k) end
        -- update dropdown values
        -- (dropdown API from earlier)
    end })
    Cu:AddButton({ Name='Delete Selected', Callback=function()
...
do
    ...
    -- custom name using a simple button input for mobile (simulate)
    Mi:AddButton({ Name='Set Custom Name (console)', Callback=function()
        local newName = tostring(math.random(10000,99999))
        S.CustomName = newName
        HSHub:Notify('Custom name set: '..newName, 'ok', 2)
    end })
...
-- AutoDrink: noted broken in LUNAR (lake-detection issue)
task.spawn(function()
    while true do
        task.wait(0.1)
...
task.spawn(function()
    while true do
        ...
        if artOn and shrineName and shrineModel then pcall(function()
            ...
            -- Step 1: find nearest carcass-type food
...
-- Auto Server Hop (Artifacts tab Recommend)
task.spawn(function()
    while true do
        task.wait(30)
...
-- AutoAcceptNest: loop other players, fire AcceptRequest
task.spawn(function()
    while true do
        task.wait(5)
...
-- AlwaysLayEffect: Lay:FireServer(true)
task.spawn(function()
    while true do
        task.wait(1)
...
-- AutoAggression: StateAilment:FireServer("Aggression")
task.spawn(function()
    while true do
        task.wait(1)
...
-- AutoScentHidden: HideScent:FireServer()
task.spawn(function()
    while true do
        task.wait(1)
...
-- AutoCowerState: StateAilment:FireServer("Cower")
S.AutoCowerStateValue = false
task.spawn(function()
    while true do
        task.wait(1)
...
-- AutoNestValue: TP + Nest:FireServer when Age > 66
...
-- Final notification with writefile fix info
local executor = "unknown"
pcall(function() executor = identifyexecutor() or "unknown" end)
HSHub:Notify('HS Hub loaded · Creatures of Sonaria · HS-COS-V4-FIX · Writefile enabled for ' .. tostring(executor), 'ok', 4)
