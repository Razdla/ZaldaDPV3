...
-- ─── Tab 4: ARTIFACTS ───────────────────────────────────────────────
S.AutoArtifactFarm = false
do
    local Tab = Window:CreateTab('Artifacts', '✦')
    local A = Tab:CreateSection('ARTIFACTS AUTOFARM')
    A:AddLabel('Requires 20+ creatures on account')
    A:AddToggle({ Name='Auto Artifact Farm', Key='AAF', Default=false,
        Tip='Cycles creatures via StoreActive + CreateSlot (LUNAR pattern)',
        Callback=function(v) S.AutoArtifactFarm=v end })
end
...
-- Main cycling loop — handles AutoFarmMutations, AutoFarmTraits, AutoArtifactFarm
task.spawn(function()
    while true do
        task.wait(1)
        local doCycle = S.AutoMutations or S.AutoTraits or S.AutoArtifactFarm
        if doCycle then pcall(function()
            ...
            -- AutoArtifactFarm: cycle unconditionally (no target match)
            -- AutoMutations/Traits: cycle until target match found
            local matched = false
            if S.AutoMutations or S.AutoTraits then
                matched = creatureMatchesTarget(creature, S.MutationTarget, S.TraitTarget)
                if matched then return end  -- target found, stop cycling this iteration
            end

            -- Trigger HUD save before cycling
            ...
            -- Cycle: store current creature, create new slot from cached Dino value
            ...
            if storeR and createR then
                ...
                task.wait(0.5)
                -- Find a Dino value to recreate from (uses creature's stored Dino attribute)
...
-- AlwaysLayEffect: Lay:FireServer()
task.spawn(function()
    while true do
        task.wait(1)
        if S.AlwaysLayEffect then
            pcall(function()
                local c = getChar()
                if c then
                    pcall(function() c:SetAttribute('Lay', true) end)
                end
                fire('Lay')
            end)
        end
