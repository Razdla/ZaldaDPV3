...
-- ═══════════════════════════════════════════════════════════════════
--   REMOTE HELPER — Sample 10 puts remotes in LocalPlayer.Remotes folder
-- ═══════════════════════════════════════════════════════════════════
local function getRemotesFolder(player)
    player = player or LP
    return player:FindFirstChild('Remotes')
end

local function getRemote(name, player)
    local f = getRemotesFolder(player)
    if f then return f:FindFirstChild(name) end
end
...
-- ═══════════════════════════════════════════════════════════════════
--   HUD HELPERS — Sample 10 reads stat values from HUDGui text labels
-- ═══════════════════════════════════════════════════════════════════
local function getHUDGui()
    return PlayerGui:FindFirstChild('HUDGui')
end
...
local function hudStatText(stat)
    ...
    local ok, val = pcall(function()
        return h.BottomFrame.Other[stat].HoverLabel.Text
    end)
...
-- ═══════════════════════════════════════════════════════════════════
--   NEAREST-TARGET FINDERS (mirror original chunk 3 helpers)
-- ═══════════════════════════════════════════════════════════════════
local function findNearestByName(folder, namePattern)
    local r = getRoot(); if not r then return nil end
    local closest, dist = nil, math.huge
    for _, obj in ipairs(folder:GetDescendants()) do
        if obj:IsA('BasePart') and obj.Name:lower():find(namePattern:lower(), 1, true) then
...
local function findNearestDrink()
    -- DrinkRemote target: nearest water source. Game stores them under workspace.Interactions.WaterSources
    local ws = Workspace:FindFirstChild('Interactions')
    if ws and ws:FindFirstChild('WaterSources') then
        return findNearestByName(ws.WaterSources, 'Water')
    end
    return nil
end
...
local function findNearestFood()
    local ws = Workspace:FindFirstChild('Interactions')
    if ws and ws:FindFirstChild('Food') then
        local r = getRoot(); if not r then return nil end
        local closest, dist = nil, math.huge
        for _, m in ipairs(ws.Food:GetChildren()) do
            local food = m:FindFirstChild('Food')
            if food and food:IsA('BasePart') then
                local d = (food.Position - r.Position).Magnitude
...
local function findNearestMud()
    local ws = Workspace:FindFirstChild('Interactions')
    if ws and ws:FindFirstChild('Mud') then
        return findNearestByName(ws.Mud, 'Mud')
    end
    return nil
end
...
local Window = HSHub:CreateWindow({
    ...
    Tag='HS-COS-V3', ToggleKey='RightShift',
})
...
do
    ...
    M:AddButton({ Name='Get Max Storage Slots',
        Callback=function() fire('GetMaxStorage'); HSHub:Notify('Max storage requested', 'ok', 2) end })

    local L = Tab:CreateSection('LOCALPLAYER')
    L:AddToggle({ Name='Auto Scent Hidden', Key='ASH', Default=false,
        Callback=function(v) S.AutoScentHidden=v end })
    L:AddToggle({ Name='Instant Lobby Return', Key='ILR', Default=false,
        Callback=function(v) S.InstantLobbyReturn=v end })
    L:AddToggle({ Name='Always Keen Observer', Key='AKO', Default=false,
        Callback=function(v) S.AlwaysKeenObserver=v end })
    L:AddToggle({ Name='Always Lay Effect', Key='ALE', Default=false,
    ...
    N:AddToggle({ Name='Auto Nest Upgrade', Key='ANU', Default=false,
        Callback=function(v) S.AutoNestUpgrade=v end })
    N:AddToggle({ Name='Enable Auto Nest', Key='EAN', Default=false,
    ...
    N:AddButton({ Name='Teleport To Nest', Callback=function()
        fire('NestRequestRemote', 'TeleportToNest')
    end })
    N:AddButton({ Name='Re-spawn Nest', Callback=function()
        fire('NestRequestRemote', 'Respawn')
    end })
end
...
do
    ...
    C:AddToggle({ Name='Auto Aggressive State', Key='AAGGR', Default=false,
        Callback=function(v) S.AutoAggressive=v end })
    C:AddToggle({ Name='Auto Scared State', Key='ASC', Default=false,
        Callback=function(v) S.AutoScared=v end })
    C:AddToggle({ Name='Anti Broken Leg', Key='ABL', Default=false,
        Callback=function(v) S.AntiBrokenLeg=v end })
    C:AddToggle({ Name='Anti Shredded Wings', Key='ASW', Default=false,
        Callback=function(v) S.AntiShreddedWings=v end })
    C:AddToggle({ Name='Anti Confusion', Key='ACO', Default=false,
        Callback=function(v) S.AntiConfusion=v end })
    C:AddToggle({ Name='Anti Grab', Key='AGB', Default=false,
        Callback=function(v) S.AntiGrab=v end })
    C:AddToggle({ Name='Infinite Stamina', Key='INFS', Default=false,
        Callback=function(v) S.InfStamina=v end })

    local CS = Tab:CreateSection('CUSTOM STATS')
    CS:AddSlider({ Name='Turn Radius', Key='TR', Min=0, Max=200, Default=0,
        Decimals=0, Callback=function(v) S.TurnRadius=v end })
    CS:AddToggle({ Name='Enable Custom Turn Radius', Key='ETR', Default=false,
        Callback=function(v) S.EnableTurnRadius=v end })
    CS:AddSlider({ Name='Walk Speed', Key='WS', Min=0, Max=200, Default=30,
        Decimals=0, Callback=function(v) S.WalkSpeed=v end })
    CS:AddToggle({ Name='Enable Custom Walk Speed', Key='EWS', Default=false,
        Callback=function(v) S.EnableWalkSpeed=v end })
    CS:AddSlider({ Name='Sprint Speed', Key='SS', Min=0, Max=300, Default=115,
        Decimals=0, Callback=function(v) S.SprintSpeed=v end })
    CS:AddToggle({ Name='Enable Custom Sprint Speed', Key='ESS', Default=false,
        Callback=function(v) S.EnableSprintSpeed=v end })
    CS:AddSlider({ Name='Fly Speed', Key='FS', Min=0, Max=200, Default=40,
        Decimals=0, Callback=function(v) S.FlySpeed=v end })
    CS:AddToggle({ Name='Enable Custom Fly Speed', Key='EFS', Default=false,
...
do
    ...
    Sv:AddToggle({ Name='Auto Eat', Key='AE', Default=false,
    ...
    Sv:AddToggle({ Name='Auto Mud Roll', Key='AMR', Default=false,
        Callback=function(v) S.AutoMudRoll=v end })

    local T = Tab:CreateSection('TOKEN AUTOFARM')
    T:AddToggle({ Name='Auto Gacha Tokens', Key='AGT', Default=false,
    ...
    MT:AddToggle({ Name='Auto Mutation(s)', Key='AMUT', Default=false,
    ...
    R:AddButton({ Name='Refresh Creature List', Callback=function()
        HSHub:Notify('Creature list refreshed', 'ok', 2)
    end })
    R:AddToggle({ Name='Auto Spawn', Key='ASP', Default=false,
        Callback=function(v) S.AutoSpawn=v end })
    R:AddSlider({ Name='Death Points Target', Key='DPT', Min=0, Max=5000, Default=1200,
        Suffix=' pts', Decimals=0, Callback=function(v) S.DeathPointsTarget=v end })
...
do
    ...
    A:AddLabel('Warning: Your account needs to have 20+ creatures')
end
...
do
    ...
    local locs, sel = {}, ''
    Cu:AddDropdown({ Name='Custom Location', Key='CL', Default='', Values={''},
...
do
    ...
    O:AddButton({ Name='Teleport to Abandoned Egg', Callback=function()
        ...
        if i and i:FindFirstChild('AbandonedEggs') then
            local closest = findNearestByName(i.AbandonedEggs, 'Egg')
            if closest then
                local root = getRoot(); if root then root.CFrame=CFrame.new(closest.Position+Vector3.new(0,5,0)) end
...
do
    ...
    if Mi.AddTextbox then
        Mi:AddTextbox({ Name='Custom Name', Default='', Placeholder='Enter name',
    ...
    local Di = Tab:CreateSection('DISCORD')
    Di:AddButton({ Name='Copy Discord Link', Callback=function()
    ...
    AL:AddLabel('Config saving uses HSHub built-in config IO')
    AL:AddButton({ Name='Save Config', Callback=function() HSHub:Notify('Config saved','ok',2) end })
    AL:AddButton({ Name='Reset Config', Callback=function() HSHub:Notify('Config reset','ok',2) end })
...
-- ═══════════════════════════════════════════════════════════════════
--   FEATURE LOOPS (exact patterns from chunk3_pretty.lua)
-- ═══════════════════════════════════════════════════════════════════

-- AutoDrink: while AutoDrink and not AutoMissions and HUD Thirst != 100% -> DrinkRemote:FireServer(nearestDrink)
task.spawn(function()
    while true do
        task.wait(0.1)
        if S.AutoDrink and not S.AutoMissions then
            pcall(function()
                if getChar() and hudStatText('Thirst') ~= '100%' then
                    local target = findNearestDrink()
                    fire('DrinkRemote', target)
                end
            end)
        end
    end
...
-- AutoEat: TP to nearest food + spam Food:FireServer until Hunger=100% or food gone
task.spawn(function()
    local savedCF
    while true do
        task.wait(1)
        if S.AutoEat and not S.AutoMissions then
            pcall(function()
                ...
                if char and hudStatText('Hunger') ~= '100%' then
                    ...
                    if food and food:FindFirstChild('Food') then
                        local v = food.Food.Position
                        local root = getRoot()
                        if root then
                            ...
                            until hudStatText('Hunger') == '100%' or not S.AutoEat
                                or not food.Parent
...
-- AutoMudRoll: TP to mud + spam Mud:FireServer until Muddy attribute
task.spawn(function()
    local savedCF
    while true do
        task.wait(0.1)
        if S.AutoMudRoll and not S.AutoMissions then
            pcall(function()
                ...
                if char and not char:GetAttribute('Muddy') then
                    ...
                    if mud then
                        local target = mud.Position + Vector3.new(0, mud.Size.Y/2, 0)
                        local root = getRoot()
                        if root then
                            if not savedCF then savedCF = root.CFrame end
                            repeat
                                ...
                                fire('Mud', mud)
                            until char:GetAttribute('Muddy') or not S.AutoMudRoll
...
-- AutoCowerStateValue: StatAilment:FireServer('Cower') when not already cowering
task.spawn(function()
    while true do
        task.wait(1)
        if S.AutoCowerStateValue then
            pcall(function()
                local char = getChar()
                if char and not char:GetAttribute('Cower') then
                    fire('StatAilment', 'Cower')
                end
            end)
        end
    end
    ...
    while true do
        ...
        if S.AutoShelter then
            pcall(function()
                ...
                if c and c == Color3.fromRGB(255,0,0) then
                    fire('Sheltered', true)
                end
            end)
        end
...
-- AutoSelfKill: LavaSelfDamage:FireServer() when DeathPoints >= target
task.spawn(function()
    while true do
        task.wait(1)
        if S.AutoSelfKill then
            pcall(function()
                if getChar() then
                    -- DeathPoints accessor (assume available via HUDGui module)
                    -- If not accessible, fallback to firing unconditionally with task.wait gate
                    S.NoLavaDamage = false  -- disable lava protection
                    fire('LavaSelfDamage')
                end
            end)
        end
...
-- AutoFarmMutations / AutoFarmTraits: RestartSlotRemote:InvokeServer(slot, false) on dead creature
task.spawn(function()
    while true do
        task.wait(0.1)
        if S.AutoMutations or S.AutoTraits then
            pcall(function()
...
-- AutoGachaTokens: GetSpawnedTokenRemote:InvokeServer()
task.spawn(function()
    while true do
        task.wait(0.1)
...
-- AutoAcceptNest: NestRequestRemote:FireServer('AcceptRequest') for all players
task.spawn(function()
    while true do
        task.wait(5)
                ...
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= LP and p:FindFirstChild('Settings') then
                        ...
                        if n and (n.Value == S.InvitationType or S.InvitationType == 'Everyone') then
                            local r = p:FindFirstChild('Remotes')
                            if r and r:FindFirstChild('NestRequestRemote') then
                                pcall(function() r.NestRequestRemote:FireServer('AcceptRequest') end)
                            end
                        end
                    end
                end
...
-- AutoSpawn: respawn dead slots via RestartSlotRemote
task.spawn(function()
    while true do
        task.wait(1)
        if S.AutoSpawn then
...
-- Custom stats (Walk/Sprint/Fly Speed)
task.spawn(function()
    while true do
        task.wait(0.5)
        local h = getHumanoid()
...
-- Visual: RemoveFog (continuous in case game overrides)
task.spawn(function()
    while true do
        task.wait(2)
        if S.RemoveFog then
...
HSHub:Notify('HS Hub loaded · Creatures of Sonaria · HS-COS-V3', 'ok', 3)
